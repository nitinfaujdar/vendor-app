...
class VendorPerformanceView(GenericAPIView):
    ...
    def get(self, request, vendor_id):
        ...
        return Response({"message": "Vendor performance metrics retrieved successfully", "data": performance_data}, 
                        status=status.HTTP_200_OK)

class AcknowledgePurchaseOrderView(GenericAPIView):
    serializer_class = PurchaseOrderSerializer

    def post(self, request, po_id):
        purchase_order = get_object_or_404(PurchaseOrder, id=po_id)

        # Your logic to update acknowledgment_date and trigger recalculation
        # of average_response_time goes here.

        serializer = self.get_serializer(purchase_order, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()

        return Response({"message": "Purchase Order acknowledged successfully", "data": serializer.data}, 
                        status=status.HTTP_200_OK)
